...
class CheckLayerSat(object):
    ...
    def get_layer_from_submodule(
        self, submodule: torch.nn.Module, layers: dict, name_prefix: str = ""
    ):
    ...
    def _record_stat(
        ...
        if activations_batch.dim() == 4:  # conv layer (B x C x H x W)
            if self.interpolation_strategy is not None and (
                activations_batch.shape[3] > self.interpolation_downsampling
                or activations_batch.shape[2] > self.interpolation_downsampling
            ):
            ...
            if self.conv_method == "median":
                ...
                activations_batch, _ = torch.median(
                    reshaped_batch, dim=2
                )  # channel median
        ...
        if layer.name not in self.logs[f"{training_state}-{stat}"]:
            ...
            self.logs[f"{training_state}-{stat}"][layer.name] = TorchCovarianceMatrix(
                device=self.device, save_data=save_data
            )
    ...
    def register_forward_hooks(self, layer: torch.nn.Module, stats: list):
        ...
        def record_layer_saturation(layer: torch.nn.Module, input, output):
            ...
            if (
                ...
                or self.seen_samples[training_state][layer.name] < self.max_samples
                ...
                num_samples = (
                    ...
                    else output.data.shape[0]
                )
                activations_batch = output.data[:num_samples]
                self.seen_samples[training_state][layer.name] += num_samples
                if self.verbose:
                    print(
                        "seen {} samples on layer {}".format(
                            self.seen_samples[training_state][layer.name], layer.name
                        )
                    )
                ...
                if "lsat" in stats:
                    ...
                    if activations_batch.dim() == 4:  # conv layer (B x C x H x W)
                        if self.conv_method == "median":
                            ...
                            activations_batch = torch.mean(
                                activations_batch, dim=(2, 3)
                            )
                        elif self.conv_method == "flatten":
                            activations_batch = activations_batch.view(
                                activations_batch.size(0), -1
                            ...
                            shape = reshaped_batch.shape
                            reshaped_batch: torch.Tensor = reshaped_batch.flatten(1)
                            reshaped_batch: torch.Tensor = reshaped_batch.permute(
                                [1, 0]
                            )
                            activations_batch = reshaped_batch

                    if layer.name in self.logs[f"{training_state}-saturation"]:
                        self.logs[f"{training_state}-saturation"][layer.name].update(
                            activations_batch
                        )
                    else:
                        self.logs[f"{training_state}-saturation"][
                            layer.name
                        ] = TorchCovarianceMatrix(device=self.device)
                        self.logs[f"{training_state}-saturation"][layer.name].update(
                            activations_batch
                        )
            ...
            if "-saturation" in key:
                for i, layer_name in enumerate(self.logs[key]):
                    ...
                    if self.logs[key][layer_name]._cov_mtx is None:
                        raise ValueError(
                            "Attempting to compute saturation when covariance is not initialized"
                        )
                    sat = compute_saturation(
                        self.logs[key][layer_name].fix().cpu().numpy(),
...
class CheckLayerMI(object):
    """Takes PyTorch layers, layer or model as `modules` and writes tensorboardX
    summaries to `logging_dir`. Outputs mutual information between the layer and input with call to self.mi().
    ...
    def __init__(
        ...
        max_samples=None,
        stats: list = ["lsat"],
        layerwise_mi: bool = True,
        reset_covariance: bool = True,
        average_mi: bool = False,
        ignore_layer_names: List[str] = [],
        include_conv: bool = True,
        conv_method: str = "channelwise",
        mi_threshold: str = 0.99,
        verbose=False,
        device="cuda:0",
    ):
        self.verbose = verbose
        ...
        self.layerwise_mi = layerwise_mi
        self.average_mi = average_mi
        self.ignore_layer_names = ignore_layer_names
        self.logs = {
            "eval-mi": OrderedDict(),
            "train-mi": OrderedDict(),
        }
        self.seen_samples = {"train": {}, "eval": {}}
        self.global_steps = 0
        self.global_hooks_registered = False
    ...
    def __getattr__(self, name):
        if name.startswith("add_") and name != "add_mi":
            return getattr(self.writer, name)
        else:
            try:
                # Redirect to writer object
    ...
    def register_forward_hooks(self, layer: torch.nn.Module, stats: list):
        ...
        def record_layer_mi(layer: torch.nn.Module, input, output):
            """Hook to register in `layer` module."""
            ...
            if (
                ...
                if "lmi" in stats:
                    ...
                    if layer.name in self.logs[f"{training_state}-mi"]:
                        self.logs[f"{training_state}-mi"][layer.name].update(
                            activations_batch
                        )
                    else:
                        self.logs[f"{training_state}-mi"][
                            layer.name
                        ] = TorchCovarianceMatrix(device=self.device)
                        self.logs[f"{training_state}-mi"][layer.name].update(
                            activations_batch
        ...
        layer.register_forward_hook(record_layer_mi)

    def add_mutual_information(self, data, layer_size, mine_hidden_size, save=True):
        """
        Computes Mutual Information and saves all stats
        :return:
        """
            ...
            if "-mi" in key:
                for i, layer_name in enumerate(self.logs[key]):
                    ...
                    mi = compute_mutual_information(
                        mine_model,
                        x_sample=inp,
                        y_sample=self.logs[key][layer_name].fix().cpu().numpy(),
                        thresh=self.threshold,
                        epochs=epochs,
                    )
                    self.seen_samples[key.split("-")[0]][layer_name] = 0

                    if self.layerwise_mi:
                        name = key + "_" + layer_name
                        self.writer.add_scalar(name, mi)
                    if "eval" in key:
                        val_mis.append(mi)
                    elif "train" in key:
                        train_mis.append(mi)
        ...
        if self.average_sat:
            self.writer.add_scalar("average_train_sat", np.mean(train_mis))
            self.writer.add_scalar("average_eval_sat", np.mean(val_mis))

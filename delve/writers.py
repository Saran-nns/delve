...
class CSVWriter(AbstractWriter):
    ...
    def resume_from_saved_state(self, initial_epoch: int):
        ...
        if self._check_savestate_ok(self.savepath + ".csv"):
            self.value_dict = pd.read_csv(
                self.savepath + ".csv", sep=";", index_col=0
            ).to_dict("list")
...
class CSVandPlottingWriter(CSVWriter):
    def __init__(
        ...
        **kwargs,
        ...
        self.plot_man_func = (
            plot_manipulation_func
            if plot_manipulation_func is not None
            else lambda x: x
        )
        self.primary_metric = (
            None if not "primary_metric" in kwargs else kwargs["primary_metric"]
        )
...
def extract_layer_stat(
    df, epoch=19, primary_metric=None, stat="saturation"
    ...
    train_cols = [
        col for col in cols if "train" in col and not "accuracy" in col and stat in col
    ]
...
def plot_stat(
    ...
    plt.title(
        pathlib.Path(savepath)
        .name.replace("_", " ")
        .replace(".csv", f" epoch: {epoch}"),
        fontsize=fontsize,
    )
    if show_col_label_y:
        plt.ylabel(
            stat if not stat in STATMAP else STATMAP[stat],
            rotation="vertical",
            fontsize=fontsize,
        )
    if show_grid:
...
def plot_stat_level_from_results(
    savepath,
    epoch,
    stat,
    primary_metric=None,
    ...
    epoch_df, pm = extract_layer_stat(
        df, stat=STATMAP[stat], epoch=epoch, primary_metric=primary_metric
    )
    ax = plot_stat(
        df=epoch_df,
        pm=pm,
